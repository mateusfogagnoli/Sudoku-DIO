# Compile and run the Sudoku GUI (UIMain) with a preconfigured puzzle.
# Run this from PowerShell: `./run.ps1` or `powershell -ExecutionPolicy Bypass -File .\run.ps1`

# Move to script directory (project root)
Set-Location -Path $PSScriptRoot

# Collect Java sources and compile
Get-ChildItem -Path .\src -Recurse -Filter *.java | ForEach-Object FullName | Set-Content sources.txt
cmd /c "javac -d out @sources.txt"

# Puzzle arguments (each string is a separate JVM argument)
$arguments = @(
...
# Execute the UI main with the arguments
java -cp out br.com.dio.UIMain @arguments
